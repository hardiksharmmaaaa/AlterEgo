...
const pendingCopy: Record<Exclude<PortraitJobStatus, "completed">, { title: string; detail: string }> = {
  queued: {
  ...
  failed: {
    ...
    detail: "Return to the booth to retry, or ask a volunteer for help.",
  },
};
...
export default function ResultPortal({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setJob((await response.json()) as PublicPortraitJob);
  }, [token]);
  ...
  useEffect(() => {
    void load();
    if (job?.status === "completed" || job?.status === "failed" || unavailable) return;
    ...
    return (
      <main className="portal-shell centered-screen">
        ...
        <section className="portal-message">
          ...
          <p>{deleted ? "The locally stored source photo, generated portrait, and private result access have been cleared." : "Ask a volunteer for help if you are still at the event."}</p>
        </section>
      </main>
    );
  }
  ...
  const expiry = job
    ...
    : "";
  const portalTheme = getTheme(job?.theme);

  return (
    <main
      className={`portal-shell portal-shell--${portalTheme.id}`}
      style={{ "--theme-accent": portalTheme.accent } as React.CSSProperties}
    >
      <header className="portal-header"><BrandMark /><span className="demo-badge">Private result</span></header>
      {!ready ? (
        <section className="portal-wait" aria-live="polite">
          ...
          <h1>{job?.errorCode === "billing_required" ? "This booth needs an organizer." : job ? pendingCopy[job.status as Exclude<PortraitJobStatus, "completed">].title : "Opening your private link…"}</h1>
          <p>{job?.error ?? (job ? pendingCopy[job.status as Exclude<PortraitJobStatus, "completed">].detail : "This will only take a moment.")}</p>
        </section>
      ) : (
        <section className="portal-result">
          <div className={`final-portrait final-portrait--${job.theme}`}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={job.resultUrl!} alt={`Your ${job.themeName} AI portrait`} />
            <div className="portrait-brand"><BrandMark compact /><span>{job.themeName} · AI portrait</span></div>
          </div>
          <div className="portal-copy">
            <p className="micro-label">{job.themeName} universe</p>
            <h1>Your alter ego is ready.</h1>
            <p>Save the portrait to your phone before this private link expires.</p>
            <p className="expiry-copy">Available until {expiry}</p>
            <a className="primary-button" href={job.resultUrl!} download={`ku-alter-ego-${job.theme}.jpg`}><DownloadIcon /> Download photo</a>
            {managementToken && (
              <div className="danger-zone">
                {!confirmDelete ? (
                  ...
                  <div className="delete-confirm">
                    <p>This removes the locally stored source and generated portrait, then disables this link. This cannot be undone.</p>
